...
export default {
  ...
  created() {
    axios.get("/api/user/stripe/payment-methods").then((response) => {
  ...
  async mounted() {
    ...
    continue_button.addEventListener("click", () => {
      ...
      axios
        .post(BASE_URL + "/api/coinbase/create/charge", { amount: this.amount })
        .then((response) => {
          this.coinbase_payment_url = response.data.data.hosted_url;
        });
    });

    const stripe = Stripe("pk_test_d1AYVSPvMyUX7ie2tzQC70UA007ViAIJIm");

    await axios.get(BASE_URL + "/api/payment-intent").then((response) => {
    ...
    form.addEventListener("submit", async (event) => {
      ...
      await stripe
        ...
        .then((result) => {
          console.log(result);
          axios
            .post(BASE_URL + "/api/stripe/add-payment-method", {
